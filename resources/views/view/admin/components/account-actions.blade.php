<div class="flex items-center gap-2">
  ...
  {{-- LATE --}}
  <button class="late-balance" data-user='@json($d)'>
  ...
  @if ($d->email !== 'superadmin@sangnila.com')
    @if ($d->status_account === 'active')
      <a
        href="{{ route('account.edit', ['id'=>$d->id,'status'=>'suspended']) }}"
        onclick="return confirm('Are you sure want to disable this account?');"
      >
        <img src="{{ asset('img/ban.svg') }}" alt="ban" />
      </a>
    @else
      <a
        href="{{ route('account.edit', ['id'=>$d->id,'status'=>'active']) }}"
        onclick="return confirm('Are you sure want to reenable this account?');"
      >
    ...
    <a
      href="{{ route('account.delete', ['id'=>$d->id]) }}"
