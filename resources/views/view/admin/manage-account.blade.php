...
@section ('content')
  ...
  <div
    class="container-draft bg-[#F0F3F8] p-6 rounded-lg shadow-lg overflow-x-auto"
  >
    {{-- HEADER --}}
          ...
          <tr class="{{ $loop->odd ? 'bg-white' : '' }}">
            ...
            <td>
              <div class="w-full flex items-center gap-2">
                ...
                <p class="break-words text-start">{{ $d->name }}</p>
              </div>
            </td>
            ...
            <td class="text-sm">
              {{ formatTime($d->overtime_balance) }}
            </td>

            <td class="text-sm">
              {{ formatTime($d->leave_balance) }}
            </td>
      ...
      @forelse ($data as $d)
        <div x-data="{ open: false }" class="bg-white rounded-lg p-4 shadow">
          ...
          <div
            ...
            @click="open = !open"
          >
            <div class="font-semibold text-[#012967]">{{ $d->name }}</div>
  ...
  <script>
    ...
    function openEditModal(btn) {
      ...
      window.dispatchEvent(
        new CustomEvent("open-modal", { detail: "edit-modal" }),
      );
    }
    ...
    function openLateModal(btn) {
      ...
      $("#late-modal-user-leave-balance").text(
        `Available: ${leaveDay} day(s) ${leaveHour} hour(s)`,
      );

      $("#late-modal-user-overtime-balance").text(
        `Available: ${overtimeDay} day(s) ${overtimeHour} hour(s)`,
      );

      window.dispatchEvent(
        new CustomEvent("open-modal", { detail: "late-modal" }),
      );
    }
