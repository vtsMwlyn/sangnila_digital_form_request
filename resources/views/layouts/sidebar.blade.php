...
{{-- mobile --}}
 ...
 >
    ...
    <aside
        :class="open ? 'translate-x-0' : '-translate-x-full'"
        class="fixed top-0 left-0 h-full w-72 bg-white text-white shadow-xl transform transition-transform duration-300 ease-in-out z-50 flex flex-col items-center"
        style="background-image: url('{{ asset('img/sidebar-bg.webp') }}'); background-size: cover; background-position: center;"
    >

        <!-- Profile -->
        <div class="mt-16 mb-6 w-full py-6 hover:bg-white/10">
            @if(Auth::user()->profile_photo)
                <a href="{{ route('profile.edit') }}">
                    <img
                        src="{{ asset(Auth::user()->profile_photo) }}"
                        class="w-[120px] h-[120px] rounded-full mx-auto object-cover shadow-md"
                    />
                </a>
            @else
        ...
        <h2 class="text-white font-bold text-lg mb-8 drop-shadow">
            Hi, {{ Auth::user()->name }}!
        </h2>

        <!-- NAV ITEMS -->
        <nav class="w-full flex flex-col text-white">
            <a
            href="{{ route('dashboard') }}"
            class="flex items-center space-x-4 px-5 py-3
            font-semibold transition-all duration-300
    ...
    <button
        @click="open = !open"
        class="fixed top-4 left-4 z-50 bg-[#1EB8CD] text-white w-10 h-10 rounded-md flex items-center justify-center shadow-lg transition"
    >
        <svg
            :class="open ? 'rotate-180' : ''"
            class="w-6 h-6 transition-transform duration-300"
