...
@section ('content')
  ...
  <div class="w-full flex flex-col gap-3 p-3 xl:p-0">
    ...
    <div class="w-full flex flex-col">
      {{-- Upper row cards --}}
      <div
        class="w-full flex flex-col xl:grid xl:grid-cols-4 xl:grid-rows-2 gap-4 xl:gap-8"
      >
        <div class="w-full bg-[#F0F3F8] rounded-2xl shadow-md p-6 relative">
          <small
            class="text-[#012967] font-semibold flex items-center justify-between text-[15px]"
          >
            {{ __('Approved Request') }}
            <i class="bi bi-check-circle-fill text-green-600 text-lg"></i>
          </small>
          <h1 class="text-3xl font-extrabold text-gray-900 py-2">
            {{ $data['approved']->count() }}
          </h1>
          <span
            class="text-sm text-gray-500"
            >{{ __('Request has been approved') }}</span
          >
        </div>
        ...
        <div class="w-full bg-[#F0F3F8] rounded-2xl shadow-md p-6 relative">
          ...
          <span
            class="text-sm text-gray-500"
            >{{ __('Total submission which still under review') }}</span
          >
        </div>
        ...
        <div class="w-full bg-[#F0F3F8] row-span-2 rounded-2xl shadow-md p-6">
          <small
            class="text-[#012967] font-semibold flex items-center justify-between text-[15px] mb-3"
          >
          ...
          <a
            ...
            class="bg-[#F0F3F8] max-h-80 mt-4 relative overflow-y-auto block"
          >
                ...
                @foreach ($data['taking_leaves'] as $log)
                  <li class="text-sm text-gray-800">
                    <span class="block font-semibold"
                      >{{ $log->user->name }} ({{ $log->leave_period >= 8 ? 'Full Day' : rtrim(rtrim($log->leave_period, '0'), '.') . ' hours' }})</span
                    >
        ...
        <div class="w-full bg-[#F0F3F8] rounded-2xl shadow-md p-6 relative">
          ...
          <h1 class="text-3xl font-extrabold text-gray-900 py-2">
            {{$data['approved']->where('type', 'overtime')->count()}} {{__('Data')}}
          </h1>
          <span
            class="text-sm text-gray-500"
            >{{ __('Total overtime approved') }}</span
          >
        </div>
        ...
        <div class="w-full bg-[#F0F3F8] rounded-2xl shadow-md p-6 relative">
          ...
          <h1 class="text-3xl font-extrabold text-gray-900 py-2">
            {{$data['approved']->where('type', 'leave')->count()}} {{__('Data')}}
          </h1>
          <span
            class="text-sm text-gray-500"
            >{{ __('Total leave approved') }}</span
          >
        </div>

        <a
          href="{{ route('register') }}"
          class="flex flex-col h-full items-start bg-gradient-to-r from-[#1EB8CD] to-[#2652B8] rounded-xl p-5 shadow-lg text-white hover:from-cyan-600 hover:to-blue-800 transition"
        >
        ...
        <div
          id="data"
          class="w-full bg-[#F0F3F8] rounded-xl shadow-6xl p-6 overflow-x-auto col-span-3 xl:h-[500px]"
        >
          <x-form-filter-all-data
          ...
          <div class="overflow-auto max-h-100 xl:max-h-80">
            <table
              class="min-w-full text-left border-collapse text-sm md:text-base hidden sm:table mb-3"
            >
              <thead
                class="bg-transparent text-[#1e293b] border-b-2 border-gray-400"
              >
                <tr>
              ...
              <tbody>
                @forelse ($data['requestData'] as $d)
                  <tr class="{{ $loop->odd ? 'bg-white' : '' }}">
                    ...
                    <td>
                      <span
                        class="py-0.5 px-2 rounded-full capitalize text-white text-sm {{ $d->type === 'overtime' ? 'bg-amber-500' : 'bg-sky-500' }}"
                        >{{ $d->type }}</span
                      >
                    </td>
                    <td>
                      <div class="w-full flex items-center gap-2">
                        <div class="w-1/6">
                          @if ($d->user->profile_photo)
                            <img
                              src="{{ asset($d->user->profile_photo) }}"
                              alt="Profile Photo"
                              class="w-8 h-8 rounded-full object-cover mx-auto"
                            />
                          @else
                            <img
                              src="img/default_photo_profile.webp"
                              alt="Profile Photo"
                              class="w-8 h-8 rounded-full object-cover mx-auto"
                            />
                          @endif
                        </div>
                        <p class="w-5/6 break-words">{{ $d->user->name }}</p>
                      </div>
                    </td>
                    <td
                      class="max-w-xs"
                      title="{{ $d->reason ?? $d->task_description }}"
                    >
                    ...
                    <td>
                      ...
                      @endphp
                      <span
                        class="{{ $statusClass }} text-sm rounded-full px-2 py-0.5"
                        >{{ ucfirst($d->request_status) }}</span
                      >
                    </td>
            ...
            <div class="sm:hidden block mt-4 mb-3">
              @forelse ($data['requestData'] as $d)
                @php
                  $statusClass = match($d->request_status) {
                      'approved' => 'bg-green-500 text-white rounded-full px-3 py-1/2',
                      'review' => 'bg-gray-500 text-gray-100 rounded-full px-3 py-1/2',
                      'rejected' => 'bg-red-500 text-white rounded-full px-3 py-1/2',
                      default => 'bg-yellow-500 text-white rounded-full px-3 py-1/2',
                  };
                @endphp

                <div
                  x-data="{ open: false }"
                  class="bg-white rounded-xl shadow-md p-4 mb-3 border border-gray-200 w-full"
                >
                  {{-- HEADER --}}
                  <button
                  ...
                  >
                    <div class="w-full flex flex-col">
                      <div class="w-full flex items-center gap-2">
                        ...
                        <p class="break-words text-start">{{ $d->user->name }}</p>
                      </div>

                      {{-- Date --}}
                      <div class="mb-1 flex flex-col items-start text-sm text-slate-500">
                        <div>
                          {{ Carbon\Carbon::parse($d->start_leave ?? $d->overtime_date)->format('d F Y') }}
                        </div>
                      </div>
                      ...
                      <div class="flex items-center my-2 gap-1">
                        <span
                          class="py-1/2 px-2 rounded-full capitalize text-white
        ...
        {{-- Activity Logs --}}
        <div
          class="bg-[#F0F3F8] row-span-2 rounded-2xl shadow-md p-6 h-[500px]"
        >
          <small
            class="text-[#012967] font-semibold flex items-center justify-between text-[15px] mb-3"
          >
          ...
          <a
            href="{{ route('LogActivity.show') }}"
            class="bg-[#F0F3F8] max-h-[400px] mt-4 relative overflow-y-auto block"
          >
                ...
                @foreach ($data['logs'] as $log)
                  <li class="text-sm text-gray-800">
                    <span
                      ...
                      >{{ $log->created_at->setTimezone('Asia/Jakarta')->format('d M Y, H:i') }}</span
                    >
