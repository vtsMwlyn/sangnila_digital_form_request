  ...
  <script>
    window.addEventListener("load", function () {
      window.dispatchEvent(
        new CustomEvent("open-modal", { detail: "success-modal" }),
      );
    });
  </script>
@elseif (session('fail'))
  ...
  <div class="p-4">
    ...
    <div class="flex justify-end">
      <button
        @click="
          window.dispatchEvent(
            new CustomEvent('close-modal', { detail: 'success-modal' }),
          )
        "
    ...
    {{-- Konten utama --}}
    <div
      class="flex flex-col md:flex-row items-center justify-center p-5 text-center md:text-left"
    >
      {{-- Icon centang --}}
      ...
      <div class="mt-6 md:mt-0 md:ml-8">
        ...
        @if ($success)
          <h2
            class="xl:text-3xl text-xl font-bold text-[#012967] mb-5 capitalize"
          >
            {{$success['title']}}
          </h2>
          ...
          @endif
        @elseif ($fail)
          <h2 class="text-3xl font-bold text-[#012967] mb-5 capitalize">
            {{$fail['title']}}
          </h2>
          ...
          @if (isset($fail['time']))
            <p class="text-sm text-gray-500">
              Submitted at:
              <span class="font-medium text-gray-700"> {{$fail['time']}} </span>
            </p>
          @endif
        @endif
      </div>
