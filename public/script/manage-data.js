...
function getStatusClass(status) {
  switch (status.toLowerCase()) {
    ...
    default:
      return "bg-gray-500 text-white rounded-full px-3 py-1 text-sm font-semibold";
  }
}
...
function openChooseModal(button) {
  ...
  $("#choose-modal-requested").text(
    `${requestedDays} day(s) ${requestedHours} hour(s)`,
  );
  $("#choose-modal-user-leave-balance").text(
    `Available: ${leaveDays} day(s) ${leaveHours} hour(s)`,
  );
  $("#choose-modal-user-overtime-balance").text(
    `Available: ${overtimeDays} day(s) ${overtimeHours} hour(s)`,
  );
